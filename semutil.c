...
char formalnames[MAXARGS][MAXLINES];   /* names of formal arguments  */
int localnum;                          /* number of local variables  */
char localtypes[MAXLOCS];              /* types of local variables   */
char localnames[MAXLOCS][MAXLINES];    /* names of local variables   */
...
struct id_entry *dcl(struct id_entry *p, int type, int scope)
{
   ...
   if (level > 2 && p->i_scope == PARAM) {
     //printf("inserting param into formalnames\n");
      p->i_offset = formalnum;
       strcpy(formalnames[formalnum],p->i_name);
     ...
     //printf("inserting local into localnames\n");
      p->i_offset = localnum;
      localwidths[localnum] = p->i_width;
      strcpy(localnames[localnum],p->i_name);
      if (p->i_type & T_DOUBLE)
